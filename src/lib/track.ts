...
import type { LocalTrack, NeteaseSong, TrackInfo } from "../types/models";
...
/** 由队列项还原网易云歌曲数据（用于收藏/历史落库）。 */
export function queueItemToNeteaseSong(item: QueueItem): NeteaseSong {
  if (item.song) return item.song;
  return {
    id: item.cid,
    name: item.title,
    artist: item.author,
