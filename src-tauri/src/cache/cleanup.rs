...
pub fn cleanup(
    ...
    for entry in db::netease::list_cache_entries(&connection)? {
        let path = PathBuf::from(&entry.cache_path);
        if !path.exists() || active.contains(&path) {
            continue;
        }
    ...
    db::netease::clear_cache_entries(&connection, &deleted_paths)?;
    Ok(deleted)
}
...
pub fn migrate_cache_root(old_root: &Path, new_root: &Path) -> Result<(), ApiError> {
    ...
    db::netease::update_cache_paths(&connection, &old_prefix, &new_prefix)?;
