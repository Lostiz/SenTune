...
CREATE TABLE IF NOT EXISTS netease_tracks (
  id             INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_netease_tracks_song_id ON netease_tracks(song_id);
...
CREATE TABLE IF NOT EXISTS netease_history (
  id        INTEGER PRIMARY KEY AUTOINCREMENT,
  track_id  INTEGER NOT NULL,
  played_at INTEGER NOT NULL
);
DELETE FROM netease_history
WHERE id NOT IN (SELECT MAX(id) FROM netease_history GROUP BY track_id);
CREATE UNIQUE INDEX IF NOT EXISTS idx_netease_history_track_id
...
/// 数据库未初始化时返回 None（用于可脱离数据库运行的模块，如网易云 Cookie 预热）。
pub fn try_connection() -> Option<&'static Mutex<Connection>> {
    DB.get()
}
