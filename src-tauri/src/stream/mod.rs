...
fn run_downloader(task: &Arc<StreamTask>) -> Result<(), ApiError> {
    ...
    // 网易云 CDN 使用网易云客户端（UA/Referer/Cookie），B 站使用 B 站客户端。
    let client = if task.kind == SourceKind::Netease {
        crate::api::netease::client::netease_blocking_client()
    } else {
        api::blocking_client()
    };
...
/// 已完整缓存的网易云曲目：直接创建本地完成态任务，不联网、不重新下载。
pub fn try_start_cached_netease(
    song_id: u64,
) -> Result<Option<Arc<StreamTask>>, ApiError> {
    let connection = db::connection().lock().unwrap_or_else(|p| p.into_inner());
    let Some(track) = db::netease::get_track_by_song(&connection, song_id)? else {
        return Ok(None);
    };
    let Some(cache_path) = track.cache_path else {
        return Ok(None);
    ...
    let task = Arc::new(StreamTask {
        id: id.clone(),
        kind: SourceKind::Netease,
        bvid: format!("netease:{song_id}"),
        cid: song_id,
        title: track.title.clone(),
        audio_urls: Vec::new(),
        url_index: AtomicUsize::new(0),
...
/// 创建网易云流任务：写入 netease_tracks 表、启动后台下载器（复用现有管线）。
pub fn start_netease_stream_task(
    song: &crate::models::NeteaseSong,
    stream: &crate::models::NeteaseStream,
    cache_root: &Path,
) -> Result<Arc<StreamTask>, ApiError> {
    fs::create_dir_all(cache_root)?;
    let (cache_path, part_path) =
        cache::netease_track_paths(cache_root, song.id, stream.bitrate);
    let id = format!("s{}_{}", now_ms(), rand::random::<u32>());

    let task = Arc::new(StreamTask {
        id: id.clone(),
        kind: SourceKind::Netease,
        bvid: format!("netease:{}", song.id),
        cid: song.id,
        title: song.name.clone(),
        audio_urls: vec![stream.url.clone()],
        url_index: AtomicUsize::new(0),
        audio_id: stream.bitrate,
        codec: stream.codec.clone(),
        content_type: Mutex::new(None),
        total_size: AtomicU64::new(0),
        part_path,
    ...
    let record = db::netease::NeteaseTrackRecord {
        song_id: song.id,
        title: song.name.clone(),
    ...
    {
        let connection = db::connection().lock().unwrap_or_else(|p| p.into_inner());
        db::netease::upsert_track(&connection, &record)?;
    }
...
pub fn finalize_task(task: &Arc<StreamTask>) {
    ...
    if cache::finalize_part(&task.part_path, &task.cache_path, total) {
        ...
        let connection = db::connection().lock().unwrap_or_else(|p| p.into_inner());
        if task.kind == SourceKind::Netease {
            if let Err(error) = db::netease::mark_cached(&connection, task.cid, &path, now_secs() as i64) {
                crate::logging::error(&format!("网易云缓存标记失败：{error}"));
            }
        } else if let Err(error) =
            db::tracks::mark_cached(&connection, &task.bvid, task.cid, &path, now_secs() as i64)
        {
            crate::logging::error(&format!("缓存标记失败：{error}"));
        }
    }
}
